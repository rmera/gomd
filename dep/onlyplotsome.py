...
def plot_maps(prob_map,name, mini=None,maxi=None,tickfreq=0,title="",odpi=600,size=4, noshow=False, highlights="", timed=False):
    sizex=size
    sizey=size
    if timed:
...
if a.rows:
    plot_maps(np.array(rowsacc),"%s%s_delay_%d-%d_%d_steps_%d_blur"%(corr.replace("-",""),a.rows.replace(" ","_"),a.di,a.df,a.deltad,a.blur),mini=a.min,maxi=a.max,tickfreq=a.tickfreq,title=title,odpi=a.dpi,size=a.size*cm2inches,noshow=a.noshow,highlights=a.highlights,timed=True)
