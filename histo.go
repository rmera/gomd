...
func DistanceHistogramMatrix(mol *chem.Molecule, args []string) func(coord *v3.Matrix) []float64 {
	...
	backboneindex := make([]float64, 0, Nres)
	for i := 0; i < mol.Len(); i++ {
		at1 = mol.Atom(i)
		if at1.Name != name {
			continue
	...
	ret := func(coord *v3.Matrix) []float64 {
		for i, v := range backboneindex[:len(backboneindex)-1] {
			for j, w := range backboneindex[i+1:] {
				...
				v2 := coords.VecView(w)
				distvec.Sub(v2, v1)
				matrix.AddData(v, w, distvec.Norm(2))
			}
